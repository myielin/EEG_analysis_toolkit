...
import time
import numpy as np
from functions import init_script, split_seq
...
_, freqs = split_seq(data, ch, n, n//2, False) # fetch all frequencies from the data that the model was trained on

rt = Player(data, chunk_size=n).start()
stream = Stream(bufsize=0.5, name='MNE-LSL-Player', stype='eeg', source_id='MNE-LSL').connect()

while True:
    previous = stream.get_data(picks=ch)[0]
    time.sleep(0.5)
    now = stream.get_data(picks=ch)[0]
    ...
    n_psd = mne_now.compute_psd(fmax=100, fmin=5)
